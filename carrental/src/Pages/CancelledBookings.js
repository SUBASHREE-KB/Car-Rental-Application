...
function BookingCancelled() {
    ...
    useEffect(() => {
        axios.get('/track/cancelled')
            ...
            .catch(error => {
                console.error('There was an error fetching the booking Cancelled!', error);
            });
    }, []);

    const handleShowMore = () => {
        setDisplayCount(cancelled.length);
        setShowMore(false);
        setShowLess(true);
    };
        ...
        <div style={{ backgroundColor: '#022859', width: 'auto' }}>
            <TableContainer component={Paper} style={{ marginTop: 20 }}>
                <Table>
                    ...
                    <TableBody>
                        {cancelled.slice(0, displayCount).map((item, index) => (
                            <TableRow key={index}>
